...
//	s.stream().filter(st->st.getFee()==5000).forEach(st->System.out.println(st.getName()));
		...
		Double m = s.stream().collect(Collectors.averagingDouble(x -> x.getFee()));
		System.out.println(m);
		Double l = s.stream().mapToDouble(x -> x.getFee()).average().getAsDouble();
		System.out.println(l);
       DoubleSummaryStatistics k= s.stream().collect(Collectors.summarizingDouble(x->x.getFee()));
